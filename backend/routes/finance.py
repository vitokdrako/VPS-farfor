    ...
    try:
        ...
        return {
            ...
            "summary": {
                ...
                "carry_over_balance": _get_last_closed_month_balance(db),
            }
        }
    except Exception as e:
        import traceback
...
def _get_last_closed_month_balance(db: Session) -> float:
    """Carry-over: залишок з останнього закритого місяця (для відображення в касі)."""
    try:
        ...
        if not row or not row[0]:
            return 0.0
        import json
        rd = json.loads(row[0]) if isinstance(row[0], str) else row[0]
        # closing_cash often stored inside summary
        return float(
            rd.get("closing_cash")
            or rd.get("summary", {}).get("closing_cash")
            or rd.get("summary", {}).get("net_cash")
            or 0
        )
    except Exception:
        return 0.0
...
# ============================================================
# RESTORED ENDPOINTS — Кас, План надходжень, Борги, Звіти
# ============================================================
